...
module.exports = {
	// controller for product offer view
	getproductOffer: async (req, res) => {
		try {
			let currentPage = req.query.page ? parseInt(req.query.page) : 1;
			let numberOfDocs = 10;
			const totalproducts = await productModel.aggregate([{ $unwind: "$size" }, { $project: { "size.size": 1 } }, { $count: "total" }]);
			...
			res.render("admin/admin/productOffers", { products, productCount: totalProductsCount, totalPages, currentPage });
		} catch (error) {
			console.log(error);
		}
	},
	// controller logic for product offer apply
	...
	// controller for viewing category offer
	...
	// controller for category offer apply
	...
	// controller for viewing the coupon offer
	...
	// controller for saving the new coupon
	saveCoupon: async (req, res) => {
		try {
			...
			const issuedDate = new Date(parseInt(issuedParts[2]), parseInt(issuedParts[1]) - 1, parseInt(issuedParts[0]));
			console.log(typeof disc);
			console.log(typeof minimum);

			const expiryParts = expiry.split("/");
			const expiryDate = new Date(parseInt(expiryParts[2]), parseInt(expiryParts[1]) - 1, parseInt(expiryParts[0]));
	...
	// controller for updating the coupon
	...
	// controller for changing the coupon status
